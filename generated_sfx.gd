...
var sample_hz = 11050.0 # Keep the number of samples to mix low, GDScript is not super fast.
var pulse_hz = 440.0
var phase = 0.0
...
func _fill_buffer():
	...
	while to_fill > 0:
		playback.push_frame(Vector2.ONE * waveform.sample(phase * TAU)) # Audio frames are stereo.
